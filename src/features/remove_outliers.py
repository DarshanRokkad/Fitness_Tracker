...
df[outliers_columns[:3] + ['label']].plot.hist(by = 'label', figsize = (20, 20), layout = (4, 2))
df[outliers_columns[3:] + ['label']].plot.hist(by = 'label', figsize = (20, 20), layout = (4, 2))
...
for col in outliers_columns:
    ...
    plot_binary_outliers(dataset = dataset, col = col, outlier_col = col + '_outlier', reset_index = True)
# **observations** : here we can see less outliers
...
# Loop over all columns
dataset, outliers, x_scorces = mark_outliers_lof(df, outliers_columns)
for col in outliers_columns:
...
dataset, outliers, x_scorces = mark_outliers_lof(df[df['label'] == label], outliers_columns)
for col in outliers_columns:
    plot_binary_outliers(dataset = dataset, col = col, outlier_col = 'outlier_lof', reset_index = True)
...
for col in outliers_columns:
    for exercise in df['label'].unique():
        dataset = mark_outliers_chauvenet(df[df['label'] == exercise], col)
        dataset.loc[dataset[col + '_outlier'], col] = np.nan
        n_outliers = len(dataset) - len(dataset[col].dropna())
        print(f'Removed {n_outliers} from {col} for {exercise}')
        outliers_removed_df.loc[outliers_removed_df['label'] == exercise, col] = dataset[col]
outliers_removed_df.info()
# **observations** : We can see that some columns has null values
...
outliers_removed_df.to_pickle('../../data/interim/02_outlier_removed_chauvenet.pkl')